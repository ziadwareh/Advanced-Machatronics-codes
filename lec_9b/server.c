...
/*
 There is a problem with the typical server-client code. It is that the server is only able to do one thing at a time. It can only handle one
 client where it fully services the client, then once that client is done and it disconnects, we can move on to the other client. To fix this,
 we use multiplexing. This multiplexing technique depends on the fact that all the file descriptors (those of the client handler and the server master).
 are kept in a set called fd_set. We then just use a select function, which is a blocking function, where we only exit if a new connection request is recieved, 
 or if an existing client made a data request. This way, we can handle multiple clients without needing them to disconnect before going on to the next client
*/
...
//All the elements in this array are set to 0
...
refresh_fd_set(fd_set *fd_set_ptr){
    ...
    FD_ZERO(fd_set_ptr); //clears the file descriptors in this fd_set using its pointer
    int i = 0;
    for(; i < MAX_CLIENT_SUPPORTED; i++){
        if(monitored_fd_set[i] != -1){
            //Add the file descriptor inidcated by monitored_fd_set[i] to the fd_set using its pointer (fd_set_ptr) This marks the file descriptor as monitored or active
...
main(int argc, char *argv[])
{
    ...
    intitiaze_monitor_fd_set();
    //add 0 for the system to know that we are monitoring files that correspond to stdin
    ...
    for (;;) {
        ...
        /**
         * @brief Now this is a function that would block until we eaither recieve a connection initiation requet or a data request.
         * @param get_max_fd(1)+1 will get the maximum file descriptor and adds one to it. This is because the first argument of select must be
         * 1 more than the maximum file descriptor in the set
         * @param &readfds This is the address of the de_set that indicated the file descriptors to monitor for read activity, the select function
         * modifies them to indicate which file descriptors are ready for reading
         * 
         * The the NULLs are for monitoring fds for write activities, errors, and exceptional conditions. we are not interested in these for now, hence we set that as NULL
         * 
         */
        select(get_max_fd() + 1, &readfds, NULL, NULL, NULL);

        if(FD_ISSET(connection_socket, &readfds)){ //This checks if a specific ds in the readfds is set. In this case, we are checking if the connection_socket (the master) was the triggered once, because if so, then we have a new client trying to connect
        ...
        else if(FD_ISSET(0, &readfds)){ //This means that we received an stdin request and we need to add it to the buffer. This is probably to monitor the terminal for data
